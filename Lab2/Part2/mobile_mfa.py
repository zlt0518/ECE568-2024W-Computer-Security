...
class BioConnect:
	...
	def getAuthenticatorStatus(self):
		...
		if get_result == False:
			...
			return('')
		try:
			status_data =  json.loads(get_result.content.decode('utf-8'))

		except ValueError:
			print(headers)
			print(get_result.content)
			sys.exit("Error: unexpected error from getAuthenticatorStatus")
			

		if status_data['status'] == 'active':
			if status_data['face_status'] == 'enrolled' or status_data['eye_status'] == 'enrolled' or status_data['fingerprint_status'] == 'enrolled' or status_data['voice_status'] == 'enrolled':
				return 'active'
	...
	def sendStepup(self,
		...
		post_result = requests.post(url, data=json.dumps(data), headers=headers)

		try:
			post_status = json.loads(post_result.content.decode('utf-8'))
		
		except ValueError:
			print(headers)
			print(post_result.content)
			sys.exit("Error: unexpected error from send step up")
		
		self.user_verification_id = post_status["user_verification"]["uuid"]
	...
	def getStepupStatus(self):
		...
		url = 'https://%s/v2/user_verifications/%s' % \
			( hostname, self.user_verification_id)
		...
		try:
			...
			print(headers)
			print(get_result.content)
			sys.exit("Error: unexpected error from getStepupStatus")
	

		return status_data["user_verification"]["status"]
